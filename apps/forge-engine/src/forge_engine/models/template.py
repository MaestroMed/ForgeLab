...
class CaptionStyle(Base):
    ...
    shadow_color: Mapped[str | None] = mapped_column(String(11), nullable=True)
    shadow_offset_x: Mapped[float] = mapped_column(Float, default=2.0)
    shadow_offset_y: Mapped[float] = mapped_column(Float, default=2.0)
    highlight_color: Mapped[str | None] = mapped_column(String(9), nullable=True)
...
class Template(Base):
    ...
    # Hook card style
    hook_card_style: Mapped[dict | None] = mapped_column(JSON, nullable=True)
